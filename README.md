...
既存 CI（file 側）: `Core Suite CI`（pytest / adapter / skill manifest pointers / ADR 採番 / visibility claim 等）、`ai-ratchet-gate`、`PR日本語gate`、`PRセルフレビュー（base監査 advisory）`。required status checks は Settings。
