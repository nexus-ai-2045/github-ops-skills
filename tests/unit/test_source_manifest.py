...
def test_refresh_updates_source_sha256_for_local_root_when_paths_match(
    tmp_path: Path,
) -> None:
    target = tmp_path / "skills" / "x" / "SKILL.md"
    target.parent.mkdir(parents=True)
    ...
    assert record["target_sha256"] == digest
    assert record["sha256"] == digest


def test_refresh_leaves_non_local_root_source_sha256_untouched(
    tmp_path: Path,
) -> None:
    target = tmp_path / "skills" / "x" / "SKILL.md"
    target.parent.mkdir(parents=True)
