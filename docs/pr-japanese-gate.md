...
4. account mapと固定したbase/head SHAを渡し、`scripts/create_pr_with_japanese_gate.py --confirm`を実行する。
5. ラッパーがidentity、PRIVATE、権限、clean、local/remote head、baseを再確認する。
6. ラッパーが検査済みbody snapshotをstdinから`gh pr create --body-file -`へ渡し、作成後のtitle/body/base/headを再取得して確認する。
